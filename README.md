...
- **Job-linked spending is household-wide, not per person.** A variable cost marked job-linked (commute, childcare) drops out when *anyone's* job ends. In a two-adult household that's optimistic: one partner's commute is still being paid after the other is made redundant.
- **Surplus income is not saved.** In a month where income exceeds fixed and discretionary spending, the difference is not added to any asset. Over a notice period this makes runway look slightly shorter than it would be for a household that banks its surplus — a conservative bias, in keeping with a premortem.
- **Saved files are pinned to a fixed precision.** Money is written to the cent and ratios (haircuts, the income-reduction factor) to four decimal places, so a household file reloads to exactly the values that were saved.
